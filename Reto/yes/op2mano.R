...
puos <- function(x,y)
{
  ...
  increm <- 1.4
  errorinicial   <- 1
  erroraceptadox <- errorinicial
  erroraceptadoy <- errorinicial
  puntos <- c(1)
  
  minx    <- c(Inf,Inf,Inf,Inf,Inf)# de, medio , hasta, valor, puntos intersectantes
  ...
  primy   <- TRUE
  lenx    <- 0 # longitud del tramo actual 
  ...
  while ( top != length(x) )
  {
    if ( i - top >= 1)
    {
      retx <- findMin(x,y,top,i) 
      rety <- findMin(y,x,top,i) 
      
      if ( abs(minx[4] - retx[2]) <= erroraceptadox || primx) # si se encuentran otros valores que minimicen el error, se guardan. o si es el primer valor revisado
      {
        minx <- c( top ,retx[1], i, retx[2],retx[3])
        prim = FALSE
        erroraceptadox <- erroraceptadox + increm*(lenx/length(x)) # para darle mas 
        lenx <- 1 # se reinicia la longitud actual 
      }
      else
      {
        lenx <- lenx + 1 # aumenta la longitud del tramo actual
      }
      if ( abs(miny[4] - rety[2]) <= erroraceptadoy || primy) # si se encuentran otros valores que minimicen el error, se guardan. o si es el primer valor revisado
      {
        ...
        prim = FALSE
        erroraceptadoy <- erroraceptadoy + increm*(leny/length(x)) # para darle mas 
        leny <- 1 # se reinicia la longitud actual 
      }
      else
      {
        leny <- leny + 1 # aumenta la longitud del tramo actual
      }
      ...
      if ( i >= length(x) )
      {
        ...
        erroraceptadox <- errorinicial # reiniciar el error aceptado
        erroraceptadoy <- errorinicial # reiniciar el error aceptado
...
puntos <- function(x,y)
{
  ...
  while ( top != length(x) )
  {
    if ( i - top >= 1)
    {
      ...
      if ( abs(miny[4] - min[4]) <= erroraceptadoy || primy) # si se encuentran otros valores que minimicen el error, se guardan. o si es el primer valor revisado
      {
        miny <- c( top ,rety[1], i, rety[2],rety[3])
        primy = FALSE
      ...
      if ( i >= length(x) )
      {
        ...
        cat("desde : ",min[1],"\tpunto medio : ",min[2], "\thasta : ",min[3], "\tpuntos intersectantes : ",min[5],"\terror : ",min[4],"\n")
        totintersect <- totintersect + min[5]
        if ( min[2] == puntos[ length( puntos ) ] ) 
        {
          puntos <- c( puntos,min[ 3 ] ) # min[1] en teoria es el ultimo de los anteriores, entonces ya esta 
        ...
        prim <- TRUE
        miny  <- c(Inf,Inf,Inf,Inf,Inf)# de, medio , hasta, valor, interseccion
        primy <- TRUE
      }
      else 
      {
